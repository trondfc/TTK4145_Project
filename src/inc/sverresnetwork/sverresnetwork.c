...
void * thr_tcpConnectionListen(void * parameter){
  ...
  if(setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &(struct timeval){.tv_sec = 5, .tv_usec = 0}, sizeof(struct timeval)) < 0){
      printf("Unable to set socket timeout\n");;
  }
  if(setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, &(struct timeval){.tv_sec = 5, .tv_usec = 0}, sizeof(struct timeval)) < 0){
      printf("Unable to set socket timeout\n");
  }
